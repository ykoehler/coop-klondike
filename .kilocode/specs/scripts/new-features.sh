...
readonly SPECS_DIR=$(get_specs_dir "$REPO_ROOT")
...
readonly NEXT=$((HIGHEST + 1))
readonly FEATURE_NUM=$(printf "%03d" "$NEXT")
...
readonly WORDS=$(echo "$BRANCH_NAME" | tr '-' '\n' | grep -v '^$' | head -3 | tr '\n' '-' | sed 's/-$//')
...
readonly FEATURE_DIR="$(get_feature_dir "$REPO_ROOT" "$BRANCH_NAME")"
mkdir -p "$FEATURE_DIR"

# Copy template if it exists
TEMPLATE="$SPECS_DIR/templates/spec-template.md"
